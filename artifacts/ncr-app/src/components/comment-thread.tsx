  ...
  return (
    <div className="mt-4 rounded-xl border border-[#F2F4F6] overflow-hidden">
      ...
      <div className="bg-[#F8F9FA] px-3 py-2 flex items-center gap-2 border-b border-[#F2F4F6]">
        <MessageSquare className="h-3.5 w-3.5 text-[#8B95A1]" />
        <span className="text-[11px] font-semibold text-[#8B95A1]">협업 의견</span>
        {comments.length > 0 && (
          <span className="ml-auto text-[10px] font-semibold bg-[#E5E8EB] text-[#4E5968] rounded-full px-2 py-0.5">
            {comments.length}
          </span>
        )}
      </div>
      ...
      {/* Comment list */}
      {comments.length === 0 ? (
        <div className="px-3 py-5 text-center text-[12px] text-[#BEC5CC]">
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-1.5 min-w-0">
                    <div className="h-6 w-6 rounded-full bg-[#E5E8EB] flex items-center justify-center shrink-0">
                      <span className="text-[10px] font-bold text-[#4E5968]">
                        {c.authorName.slice(0, 1)}
                      </span>
                    </div>
                    <span className="text-[12px] font-semibold text-[#191F28] truncate">{c.authorName}</span>
                    <span className="text-[10px] text-[#BEC5CC] shrink-0">{timeAgo(c.createdAt)}</span>
                    {c.isEdited && (
                      <span className="text-[10px] text-[#BEC5CC] shrink-0">(수정됨)</span>
                    )}
                  </div>
                  {canEdit && editingId !== c.id && (
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        ...
                        className="h-6 w-6 rounded-lg flex items-center justify-center text-[#8B95A1] hover:text-[#191F28] hover:bg-[#F2F4F6] transition-colors"
                      >
                      ...
                      {deletingId === c.id ? (
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="h-6 px-2 rounded-lg text-[10px] font-semibold bg-red-500 text-white disabled:opacity-50"
                          >
                            삭제
                          </button>
                          <button
                            onClick={() => setDeletingId(null)}
                            className="h-6 w-6 rounded-lg flex items-center justify-center text-[#8B95A1] hover:bg-[#F2F4F6]"
                          >
                            <X className="h-3 w-3" />
                          </button>
                        </div>
                        ...
                        <button
                          ...
                          className="h-6 w-6 rounded-lg flex items-center justify-center text-[#8B95A1] hover:text-red-500 hover:bg-red-50 transition-colors"
                        >
                  ...
                  <div className="mt-2 space-y-1.5">
                    ...
                    <div className="flex items-center gap-2 justify-end">
                      <button
                        ...
                        className="h-7 px-3 rounded-lg text-[11px] text-[#8B95A1] hover:text-[#191F28] hover:bg-[#F2F4F6] transition-colors"
                      >
                        취소
                      </button>
                      <button
                        ...
                        className="h-7 px-3 rounded-lg text-[11px] font-semibold bg-[#1A1A1A] text-white disabled:opacity-50 flex items-center gap-1"
                      >
                    ...
                    {c.taggedUserIds.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {c.taggedUserIds.map((uid) => {
                          ...
                          if (!name) return null;
                          return (
                            <span key={uid} className="text-[10px] font-semibold text-purple-700 bg-purple-50 border border-purple-200 rounded-full px-2 py-0.5">
                              @{name}
                            </span>
                          );
                        })}
      ...
      {/* New comment input */}
      {user && (
        <div className="border-t border-[#F2F4F6] px-3 py-3">
          {/* Tagged users chips */}
          {taggedIds.length > 0 && (
          ...
          <div className="relative">
            <textarea
              ref={textareaRef}
              className="w-full rounded-xl bg-[#F8F9FA] px-3 py-2.5 text-[13px] text-[#191F28] outline-none focus:ring-2 focus:ring-[#1A1A1A]/10 resize-none pr-10"
              rows={2}
              placeholder="의견을 입력하세요. @이름으로 담당자를 태그할 수 있습니다."
              value={body}
            ...
            <button
              ...
              className="absolute right-2 bottom-2 h-7 w-7 rounded-lg bg-[#1A1A1A] text-white flex items-center justify-center disabled:opacity-30 transition-opacity"
            >
              {createComment.isPending ? <RefreshCw className="h-3 w-3 animate-spin" /> : <Send className="h-3 w-3" />}
            </button>
            ...
            {mentionQuery !== null && filteredMentionUsers.length > 0 && (
              <div className="absolute left-0 right-0 bottom-full mb-1 z-20 bg-white border border-[#E5E8EB] rounded-xl shadow-lg overflow-hidden">
                <div className="px-3 py-1.5 border-b border-[#F2F4F6] flex items-center gap-1.5">
                ...
                {filteredMentionUsers.map((u) => (
                  <button
                    ...
                    className="w-full text-left px-3 py-2 hover:bg-[#F8F9FA] border-b border-[#F2F4F6] last:border-0 flex items-center gap-2"
                  >
                    <div className="h-5 w-5 rounded-full bg-[#E5E8EB] flex items-center justify-center shrink-0">
                      <span className="text-[9px] font-bold text-[#4E5968]">{u.displayName.slice(0, 1)}</span>
                    </div>
          ...
          <p className="mt-1 text-[10px] text-[#BEC5CC]">Ctrl+Enter로 빠르게 등록</p>
        </div>
      )}
    </div>
  );
