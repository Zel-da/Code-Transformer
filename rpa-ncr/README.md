...
> ⚠ **반드시 관리자 권한으로 실행하세요.**
> UNIERP 가 관리자로 실행되는 환경에서는 Windows UIPI 제약으로 비관리자
> 프로세스가 UNIERP 창을 조작할 수 없습니다. 이 프로그램은 UAC 승격을
> 우회하지 않고, 스스로 관리자여야 UIA 로 UNIERP 를 제어할 수 있습니다.
> 정상 경로: `시작.bat` 실행 시 UAC 프롬프트에서 `[예]` 클릭.
> 우회 경로(`python main.py` 직접 실행)는 관리자 아닌 경우 즉시 종료합니다(exit 2).
