...
- **rpa-ncr 관련 코드/폼 프로파일/DB 스키마 변경 시 `rpa-ncr/docs/RPA운영자_사용_상세.md` 를 반드시 동기 업데이트.**
  특히: 폼 프로파일 필드 순서·`tabs_before` 변경, 새 필드 추가, 라디오·드롭박스 매핑 규칙 변경,
  자동 업데이트 로직 개선, 신규 트러블슈팅 사례. 이 문서는 실사용자(RPA 운영자)가 매일 보는 문서임.
